    ...
    public void init() {
        ...
        while (portList.hasMoreElements()) {
            ...
            if (commPortId.getPortType() == CommPortIdentifier.PORT_SERIAL) {
                // 比较串口名称是否是指定串口
                if (rxtxProperties.getPort().equals(commPortId.getName())) {
                    // 串口存在
                    isExsist = true;
                    // 打开串口
                    try {
                        ...
                        serialPort.setSerialPortParams(rxtxProperties.getRate(), rxtxProperties.getDataBits(),
                                rxtxProperties.getStopBits(), rxtxProperties.getParity());
        ...
        try {
            ...
            if (inputStream.available() == 4) {
                while ((len = inputStream.read(readBuffer)) != -1) {
                    totalLength = byteArrayToInt(readBuffer);
                    inputStream.close();
                    inputStream = null;
                    break;
                }
            ...
            if (!StringUtils.isBlank(sb.toString())) {
                messageParseAble.messageParse(sb.toString(), sumLength, totalLength);
            }
        } catch (IOException e) {
